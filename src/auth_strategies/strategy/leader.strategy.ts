...
export class LeaderStrategy extends PassportStrategy(Strategy, 'leader') {
  constructor(private readonly configService: ConfigService) {
    super({
      ...
      authorizationURL: `https://leader-id.ru/apps/authorize?client_id=${configService.get('LEADER_CLIENT_ID')}&redirect_uri=${configService.get('LEADER_CLIENT_REDIRECT')}&response_type=code`,
