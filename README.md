...
- HIDDEN_SECRET - ключ шифрования скрытых методов, строка из символов произвольной длины,
- HIDDEN_METHOD - метод шифрования, по-умолчанию **MD5**,
- HIDDEN_EXPIRES - срок действия токена шифрования в секундах,
- HIDDEN_SIMPLE - токен для простой защиты скрытых методов.
...
В качестве защиты используется простая проверка токена из заголовка Authorization на соответствие строке, заданной в переменной окружения **HIDDEN_SIMPLE**.
...
- В качестве соли берется значение переменной окружения **HIDDEN_SECRET**, к нему через точку добавляется текущая временная метка.

- Полученная строка хешируется методом, заданным в переменной окружения **HIDDEN_METHOD**. По-умолчанию используется метод **MD5**, также поддерживаются **SHA1**, **SHA256**, **SHA512**.
...
const string = `${HIDDEN_SECRET}.${timestamp}`;
const method = `${HIDDEN_METHOD || 'MD5'}`.toLowerCase();
...
- В качестве соли берется значение переменной окружения **HIDDEN_SECRET**, к нему через точку добавляется полученная из токена временная метка.

- Полученная строка хешируется методом, заданным в переменной окружения **HIDDEN_METHOD**.
...
Допустимый срок годности токена берется из переменной окружения **HIDDEN_EXPIRES** как число в секундах.
...
const string = `${HIDDEN_SECRET}.${tokenTimestamp}`;
const method = `${HIDDEN_METHOD || 'MD5'}`.toLowerCase();
const hashed = hash(string, method);
...
if (HIDDEN_EXPIRES && expired > +HIDDEN_EXPIRES) {
