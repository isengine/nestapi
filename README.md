...
- SMTP_LINKS_DOMAIN - имя домена или url-адрес приложения, который будет передан в шаблон письма.
...
Далее, в защищенных методах мы должны обращаться к полю **users.id** вместо **auth.id**. Для этого мы предусмотрели в контроллерах, резолверах и сервисных методах поле **authKey**.
...
> Теперь в методе **filter** необязательно указывать параметры сортировки, в случае отсутствия, они будут заданы по-умолчанию.
...
Успешные миграции записываются в базу данных, в таблицу **migrations_typeorm**.

Название таблицы можно задать в файле **src/config/db.config.ts**, в параметре **migrationsTableName**.

При запуске миграций, будут выполнены все миграции, записей о которых нет в таблице.

При откате миграций, будут возвращены все миграции, записи о которых есть в таблице.

При синхронизации проекта, обратите внимание на это. Возможно, содержимое каталога **src/typeorm/migrations** и таблицу **migrations_typeorm** тоже нужно будет синхронизировать.
