...
- сделать, чтобы для декоратора @Auth можно было настраивать другое поле, кроме auth.id - например, users.id
таким образом, можно сделать приватные методы для сущностей, в которых таблицы связаны не через auth.id, а через другое поле, например users.id - user_id (userId)
это тянет за собой хранение поля в токенах доступа и обновления, проверку в приватных методах
или разработать другой механизм
